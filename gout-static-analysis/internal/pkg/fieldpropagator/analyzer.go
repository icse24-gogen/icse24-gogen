	...
	for _, b := range meth.Blocks {
		for _, instr := range b.Instrs {
			...
			if conf.IsSourceField(utils.DecomposeField(txType, field)) || tf.IsSourceField(txType, field) {
				//propagations = append(propagations, propagation.Taint(instr.(ssa.Node), conf, tf)) //What this is?
				propagations = append(propagations, propagation.Taint(instr.(ssa.Node)))
			}
		}
	}
