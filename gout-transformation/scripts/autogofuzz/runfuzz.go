...
func startFuzz(workpool chan string, binname string, results []*FuzzRes, wg *sync.WaitGroup, mu *sync.Mutex) {
	...
	resultDir := filepath.Join(targetPath, "result"+targetF) // relative to std/
	...
	fuzzcmdstr := fmt.Sprintf("rm lenconfig; timeout %dh go-fuzz -bin=%s.zip -procs=1", *flagFuzztime, targetF)
