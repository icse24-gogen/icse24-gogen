...
func parseFuncBodyCore(sp, ep token.Pos, filePath string) []byte {
	startPos := sp
	endPos := ep
	file, openErr := os.Open(filePath)
	if openErr != nil {
...
func parseTableDataName(fDecl *ast.FuncDecl) string {
	res := ""
	ranges := make([]*ast.RangeStmt, 0)
	baseCallCount := 0
	...
	ast.Inspect(fDecl, func(node ast.Node) bool {
		if _, isCallExpr := node.(*ast.CallExpr); isCallExpr {
			//count api call is much better
			baseCallCount += 1
		}
		return true
	})
	baseCallCount = (baseCallCount / 2)

	maxCallCount := -1
	for _, curRange := range ranges {
		curCallCount := 0
		ast.Inspect(curRange, func(node ast.Node) bool {
			if _, isCallExpr := node.(*ast.CallExpr); isCallExpr {
				curCallCount += 1
			}
			return true
		})
		if maxCallCount < curCallCount && baseCallCount < curCallCount {
			res = fmt.Sprintf("%v", curRange.X)
			maxCallCount = curCallCount
		}
	}
	return res
}
...
func parseFunc(fDecl *ast.FuncDecl, ul map[string]types.Type, el map[*types.Struct]ast.Expr, filePath string) *models.Function {
	f := &models.Function{
		...
		TableDataName: parseTableDataName(fDecl),
	}
...
func parseLocalConcreteVariables(fb *ast.BlockStmt, ul map[string]types.Type) []*models.Variable {
	...
	for _, st := range fb.List {
		...
		} /*else if _, ok := st.(*ast.RangeStmt); ok {
			count += 1
